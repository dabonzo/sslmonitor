  ...
  <div class="space-y-6">
    ...
    <div class="flex items-start justify-between">
      <div class="flex items-start space-x-3">
        ...
        <div>
          <h3 class="text-lg font-semibold text-foreground dark:text-foreground">{{ alert.title }}</h3>
          <p class="text-sm text-foreground dark:text-muted-foreground mt-1">{{ alert.message }}</p>
    ...
    <Card>
      ...
      <CardContent>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="text-sm font-medium text-foreground dark:text-muted-foreground">Website Name</label>
            <p class="font-medium">{{ alert.website_name }}</p>
          </div>
          <div>
            <label class="text-sm font-medium text-foreground dark:text-muted-foreground">URL</label>
            <p class="font-medium">
              <a :href="alert.website_url" target="_blank" class="text-primary hover:underline">
                {{ alert.website_url }}
              </a>
            </p>
          </div>
          <div v-if="alert.rule_name">
            <label class="text-sm font-medium text-foreground dark:text-muted-foreground">Triggered by Rule</label>
        ...
        <div class="space-y-4">
          <div class="flex items-start space-x-3">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-muted-foreground dark:text-muted-foreground">{{ formatDate(alert.created_at) }}</p>
              <p class="text-sm text-foreground dark:text-muted-foreground mt-1">{{ alert.message }}</p>
            </div>
          </div>
          ...
          <div v-if="alert.acknowledged" class="flex items-start space-x-3">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-muted-foreground dark:text-muted-foreground">{{ formatDate(alert.created_at) }}</p>
              <p class="text-sm text-foreground dark:text-muted-foreground mt-1">Alert was acknowledged by user</p>
            </div>
          </div>

          <!-- Simulated future events -->
          <div class="flex items-start space-x-3 opacity-50">
            <div class="rounded-full bg-muted p-1 mt-1">
              <Clock class="h-3 w-3 text-foreground" />
            </div>
            <div class="flex-1">
              <p class="text-sm font-medium">Auto-resolve (pending)</p>
              <p class="text-xs text-muted-foreground dark:text-muted-foreground">Expected in 2 hours</p>
              <p class="text-sm text-foreground dark:text-muted-foreground mt-1">Alert will auto-resolve when conditions improve</p>
            </div>
          </div>
        </div>
      </CardContent>
        ...
        <div class="space-y-3">
          ...
          >
            <div class="rounded-full bg-blue-100 dark:bg-blue-900/30 p-1 mt-0.5">
              <span class="text-xs font-bold text-primary dark:text-blue-400 w-4 h-4 flex items-center justify-center">
                {{ index + 1 }}
              </span>
            </div>
    ...
    <Card>
      ...
      <CardContent>
        <div class="space-y-2">
          <a
            href="#"
            class="flex items-center space-x-3 p-3 rounded-lg border hover:bg-muted dark:hover:bg-gray-800 transition-colors"
          >
            <ExternalLink class="h-4 w-4 text-foreground dark:text-muted-foreground" />
            <div>
              <p class="font-medium">SSL Certificate Documentation</p>
              <p class="text-sm text-foreground dark:text-muted-foreground">Learn about SSL certificate management</p>
            </div>
          </a>
          <a
            href="#"
          ...
          >
            ...
            <div>
              ...
              <p class="text-sm text-foreground dark:text-muted-foreground">Step-by-step problem resolution</p>
            </div>
          </a>
          <a
            href="#"
          ...
          >
            ...
            <div>
              ...
              <p class="text-sm text-foreground dark:text-muted-foreground">Get help from our technical team</p>
            </div>
          </a>
        </div>
      </CardContent>
  ...
  switch (key) {
    case 'days_remaining':
      if (typeof value === 'number') {
        if (value <= 3) return 'text-destructive dark:text-red-400';
        if (value <= 7) return 'text-orange-600 dark:text-orange-400';
        if (value <= 14) return 'text-yellow-600 dark:text-yellow-400';
      }
      break;
    case 'status_code':
      return value === 200 ? 'text-green-600 dark:text-green-400' : 'text-destructive dark:text-red-400';
    case 'consecutive_failures':
      return typeof value === 'number' && value > 0 ? 'text-destructive dark:text-red-400' : '';
  }
  return '';
};
